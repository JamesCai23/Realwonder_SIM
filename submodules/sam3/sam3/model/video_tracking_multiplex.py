...
class VideoTrackingMultiplex(nn.Module):
    ...
    def _get_tpos_enc(self, rel_pos_list, device, max_abs_pos=None, dummy=False):
        ...
        pos_enc = torch.tensor(rel_pos_list, device="cpu")
        if torch.device(device).type == "cuda":
            pos_enc = pos_enc.pin_memory().to(device=device, non_blocking=True)
        else:
            pos_enc = pos_enc.to(device=device)
        pos_enc = pos_enc / t_diff_max
