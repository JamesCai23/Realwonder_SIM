...
class Sam3Image(torch.nn.Module):
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        elif img_ids.dtype != torch.long:
            img_ids = img_ids.to(dtype=torch.long)
        ...
        if "backbone_fpn" in backbone_out:
            ...
            img_feats = []
            for x in vis_feats:
                cur_ids = img_ids if img_ids.device == x.device else img_ids.to(x.device)
                img_feats.append(x[cur_ids].flatten(2).permute(2, 0, 1))

            img_pos_embeds = []
            for x in vis_pos_enc:
                cur_ids = img_ids if img_ids.device == x.device else img_ids.to(x.device)
                img_pos_embeds.append(x[cur_ids].flatten(2).permute(2, 0, 1))
