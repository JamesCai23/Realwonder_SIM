...
class InferencePipeline:
    def __init__(
        ...
        # Pin the current CUDA device for the entire pipeline to avoid implicit cross-device ops.
        if self.device.type == "cuda":
            idx = self.device.index if self.device.index is not None else 0
            # Normalize device to an explicit index form (cuda:{idx}) before calling set_device.
        ...
        with self.device:
            ...
            # Ensure condition embedders live on the same device as the pipeline.
            for _name, _embedder in self.condition_embedders.items():
                if _embedder is not None:
                    _embedder.to(self.device)
    ...
    def embed_condition(self, condition_embedder, *args, **kwargs):
        if condition_embedder is not None:
            # Move all tensor inputs to the embedder's device to avoid cross-GPU issues.
            ...
            try:
                target_device = next(condition_embedder.parameters()).device
            ...
            args = tuple(_to_device(a, target_device) for a in args)
            kwargs = {k: _to_device(v, target_device) for k, v in kwargs.items()}
