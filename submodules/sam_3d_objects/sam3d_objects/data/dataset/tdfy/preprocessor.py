...
class PreProcessor:
    ...
    def _apply_transform(self, input: torch.Tensor, transform):
        if input is not None and transform is not None and transform != (None,):
            # Guard against degenerate tensors with zero spatial dims (can arise from empty masks).
            if torch.is_tensor(input) and input.ndim >= 3:
                h, w = input.shape[-2], input.shape[-1]
                if h == 0 or w == 0:
                    new_shape = list(input.shape)
                    new_shape[-2] = max(h, 1)
                    new_shape[-1] = max(w, 1)
                    input = input.new_zeros(new_shape)
