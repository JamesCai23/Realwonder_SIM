...
class FlexiCubes:
    ...
    def _compute_vd(self, voxelgrid_vertices, surf_cubes_fx8, surf_edges, scalar_field,
        ...
        vd = torch.zeros((total_num_vd, 3), device=self.device, dtype=acc_dtype)
        beta_sum = torch.zeros((total_num_vd, 1), device=self.device, dtype=acc_dtype)
        ...
        # index_add_ requires matching dtypes (autocast may produce bf16 here)
        ue_group = ue_group.to(acc_dtype)
        beta_group = beta_group.to(acc_dtype)
        ...
        '''
        if voxelgrid_colors is not None:
            vd_color = torch.zeros((total_num_vd, C), device=self.device, dtype=acc_dtype)
            ...
            uc_group = uc_group.to(acc_dtype)
