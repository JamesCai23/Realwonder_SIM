...
class FluxControlNetInpaintingPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def _get_t5_prompt_embeds(
        ...
        # Guard against tokenizer/text-encoder vocab mismatch that can surface as
        # CUDA indexSelect assertions in T5 embedding lookup.
        text_input_ids = self._sanitize_t5_input_ids(text_input_ids)
    ...
    def _sanitize_t5_input_ids(self, text_input_ids: torch.LongTensor) -> torch.LongTensor:
        try:
            input_embeddings = self.text_encoder_2.get_input_embeddings()
            vocab_size = int(input_embeddings.weight.shape[0])
        except Exception:
        ...
        min_id = int(text_input_ids.min().item())
        max_id = int(text_input_ids.max().item())
        if min_id >= 0 and max_id < vocab_size:
            return text_input_ids

        bad_mask = (text_input_ids < 0) | (text_input_ids >= vocab_size)
        bad_count = int(bad_mask.sum().item())
        fallback_id = 0
        for candidate in (self.tokenizer_2.unk_token_id, self.tokenizer_2.pad_token_id, 0):
            if candidate is not None and 0 <= int(candidate) < vocab_size:
                fallback_id = int(candidate)
                break

        sanitized_ids = text_input_ids.clone()
        sanitized_ids[bad_mask] = fallback_id
        logger.warning(
            "Detected %s out-of-range T5 token ids (min=%s, max=%s, vocab_size=%s); replaced with token id %s",
            bad_count,
            min_id,
            max_id,
            vocab_size,
