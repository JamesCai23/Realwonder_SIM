...
def _get_or_load_raft_model(device):
    cache_key = str(device)
    if cache_key not in _RAFT_MODEL_CACHE:
        _RAFT_MODEL_CACHE[cache_key] = raft.RaftOpticalFlow(device, "large")
        rp.fansi_print(f"Loaded RAFT model on {device}", "yellow")
    else:
        rp.fansi_print(f"Reusing RAFT model on {device}", "yellow")
...
def sum_indexed_values(image, index_matrix):
    ...
    if index_matrix.dtype != torch.long:
        index_matrix = index_matrix.long()
    # Defensive clamp for occasional outlier indices from iterative warp states.
    min_idx = int(index_matrix.min().item())
    if min_idx < 0:
        index_matrix = torch.clamp(index_matrix, min=0)
    ...
    safe_indices = torch.clamp(index_matrix.view(-1), min=0, max=u - 1)
    output.index_add_(0, safe_indices, flattened_pixels)
...
def indexed_to_image(index_matrix, unique_colors):
    ...
    if u <= 0:
        raise ValueError("unique_colors must contain at least one row")

    # Defensive guard: occasionally index_matrix may carry stale/outlier values due to
    # upstream numeric edge cases during iterative warping. Clamp to valid range to
    # avoid CUDA device-side assert in advanced indexing.
    if index_matrix.dtype != torch.long:
        index_matrix = index_matrix.long()
    index_matrix = torch.clamp(index_matrix, min=0, max=u - 1)
...
def get_noise_from_video(
    ...
    if device is None:
        if rp.currently_running_mac():
            ...
            # Respect CUDA_VISIBLE_DEVICES: use a visible ordinal instead of global GPU IDs.
    ...
    if isinstance(device, str) and device.startswith("cuda"):
        if not torch.cuda.is_available() or torch.cuda.device_count() == 0:
            ...
            requested_idx = 0 if len(parts) == 1 else int(parts[1])
            if requested_idx >= torch.cuda.device_count():
                device = f"cuda:{torch.cuda.current_device()}"
    
    raft_model = None
    if not input_flow:
        raft_model = _get_or_load_raft_model(device)
